...
- 로어북 로컬 열람 테마: 밝은 황백색·굵은 적갈색 판타지아, 단순 선형 암부·형광 녹색·8초 간격 배경 글리치 메카니컬, 공포 질감을 제거한 남색 도시 네온 어반 판타지로 재조정. 이미지보다 어두운 `.workspace-main`, 테마별 `책장` 제목, 네 테마 공통 진녹색·금색 `글 편집`, route 이탈 시 배경 정리를 실제 데이터 desktop/mobile `2 passed`; 생성 WebP 16–88KB, 1600×900·390×844·360×844 스크린샷에서 잘림·가로 overflow·pageerror 없음 PASS
