  ...
  <section class="wizard-shell document-workflow">
    ...
    {:else if workflowStep === 1}
      <div class="settings-grid finalization-settings">
        <section class="card stack"><h3>글의 형태</h3><label>전개 방식<select bind:value={finalRecipeId}>{#each recipes as recipe}<option value={recipe.id}>{recipe.name}</option>{/each}</select></label>{#if selectedRecipe}<p class="field-help">{selectedRecipe.description}</p>{/if}<label>결과물<select bind:value={finalOutputProfile}>{#each outputProfiles as profile}<option value={profile.key}>{profile.name}</option>{/each}</select></label><div class="grid-2"><label>시점<select bind:value={finalViewpoint}><option value="omniscient">전지적 설명자</option><option value="first_observer">1인칭 관찰자</option><option value="third_limited">3인칭 제한</option></select></label><label>시제<select bind:value={finalTense}><option value="present">현재형 중심</option><option value="past">과거형 중심</option></select></label></div><label>이번 완성본의 집필 지시<textarea bind:value={finalUserDirection} placeholder="글 만들기 때의 지시를 바꾸거나 보완할 수 있습니다."></textarea></label></section>
