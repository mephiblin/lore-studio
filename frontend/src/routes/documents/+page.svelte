<script>
  ...
  $: finalContextLabel = ({ core: '선택한 핵심만', balanced: '관련 자료까지 균형 있게', wide: '세계 맥락을 넓게', max: '가능한 자료를 최대로' }[finalContextDepth] || finalContextDepth);
    ...
    {:else if workflowStep === 1}
      <div class="output-studio final-output-studio">
        <aside class="output-specimen final-output-specimen" aria-label="완성본 결과 견본">
          <span class="specimen-mark">{finalOutputProfile === 'video_narration' ? '영상' : finalOutputProfile === 'novel_prose' ? '장면' : finalOutputProfile === 'in_universe_report' ? '기록' : '설정집'}</span>
