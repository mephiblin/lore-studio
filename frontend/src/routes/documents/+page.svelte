<script>
  ...
  const workflowSteps = [
    ...
    { short: '완성 설정', title: '완성본의 글 형태를 다시 정하세요.', copy: '처음 선택한 값은 초깃값일 뿐입니다. 이번 완성본에 맞게 모두 바꿀 수 있습니다.' },
  ...
  async function loadDocuments() {
    if (!projectId) return;
    error = ''; rememberProject(projectId);
    try {
  ...
  async function saveDocument() {
    ...
    try {
      ...
      message = '초안 정보를 저장했습니다.';
    } catch (e) { error = e.message; }
    finally { busy = ''; }
  }

  async function saveBlock(block) {
    busy = '문단 저장 중'; error = ''; message = '';
    try {
  ...
  async function extractNewCandidates() {
    if (!selected) return;
    busy = 'AI가 초안에서 새 설정 후보를 찾는 중'; error = '';
    try {
...
<div class="page">
  ...
  <section class="wizard-shell document-workflow">
    ...
    <header class="wizard-heading">
      ...
      {#if documents.length}<label class="workflow-document-select">작업할 초안<select value={selected?.id || ''} on:change={(event) => selectDocument(documents.find((item) => item.id === event.currentTarget.value) || null)}>{#each documents as document}<option value={document.id}>{document.title}</option>{/each}</select></label>{/if}
    </header>
      ...
      <div class="document-edit-stage">
        <main class="stack document-main">
          <div class="document-toolbar">
            <input class="document-title-input" aria-label="초안 제목" bind:value={selected.title} />
            <div class="row wrap"><label class="compact-status">상태<select bind:value={selected.status}><option value="draft">초안</option><option value="review">검토</option><option value="approved">완료</option><option value="archived">보관</option></select></label><button class="secondary" on:click={saveDocument}>초안 정보 저장</button><a class="ghost" href={`${API_BASE}/documents/${selected.id}/export?format=markdown`} target="_blank">초안 내보내기</a></div>
          </div>
          {#each blocks as block, index}
            <article class="card stack document-block" class:locked={block.locked}>
              <div class="document-block-header"><div class="document-block-meta"><span class="badge">{String(index + 1).padStart(2, '0')}</span><strong>{moveLabels[block.rhetorical_move] || block.rhetorical_move}</strong><span class:canon={block.certainty === 'EVIDENCED'} class:candidate={block.certainty !== 'EVIDENCED'} class="badge">{certaintyLabels[block.certainty] || block.certainty}</span></div><label class="document-lock"><input type="checkbox" bind:checked={block.locked} /> 문단 잠금</label></div>
        ...
        <aside class="stack document-inspector">
          ...
          <section class="card stack"><div class="row spread"><div><p class="eyebrow">승인 대기</p><h3 style="margin:0">새 설정 후보</h3></div><span class="badge candidate">{candidates.filter((item) => item.status === 'CANDIDATE').length}</span></div><button class="secondary" on:click={extractNewCandidates}>현재 초안에서 후보 추출</button>{#each candidates.filter((item) => item.status === 'CANDIDATE').slice(0, 5) as candidate}<div class="evidence-item"><strong>{candidate.title}</strong><small>{candidate.summary || candidate.candidate_sentence}</small><div class="row wrap" style="margin-top:8px"><button class="ghost" on:click={() => decideCandidate(candidate, 'this_document_only')}>이번 글만</button><button class="secondary" on:click={() => decideCandidate(candidate, 'save_draft')}>설정 초안으로 저장</button><button class="primary" on:click={() => decideCandidate(candidate, 'approve_canon')}>정식 설정으로 승인</button></div></div>{/each}</section>
        </aside>
      </div>
      <footer class="wizard-actions"><span></span><button class="primary" on:click={() => continueTo(1)}>완성 설정으로 계속 →</button></footer>
    {:else if workflowStep === 1}
      <div class="settings-grid finalization-settings">
        <section class="card stack"><h3>글의 형태</h3><label>집필 방식<select bind:value={finalRecipeId}>{#each recipes.filter((item) => !item.project_id || item.project_id === projectId) as recipe}<option value={recipe.id}>{recipe.name}</option>{/each}</select></label>{#if selectedRecipe}<p class="field-help">{selectedRecipe.description}</p>{/if}<label>결과물<select bind:value={finalOutputProfile}>{#each outputProfiles as profile}<option value={profile.key}>{profile.name}</option>{/each}</select></label><div class="grid-2"><label>시점<select bind:value={finalViewpoint}><option value="omniscient">전지적 설명자</option><option value="first_observer">1인칭 관찰자</option><option value="third_limited">3인칭 제한</option></select></label><label>시제<select bind:value={finalTense}><option value="present">현재형 중심</option><option value="past">과거형 중심</option></select></label></div><label>이번 완성본의 집필 지시<textarea bind:value={finalUserDirection} placeholder="글 만들기 때의 지시를 바꾸거나 보완할 수 있습니다."></textarea></label></section>
