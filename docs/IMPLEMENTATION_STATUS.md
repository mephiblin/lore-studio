...
- 세계관 자료의 Tiptap 서식 편집↔Markdown 원문 편집 양방향 전환과 AI Markdown 제안 구조화 반영. 기존 `body_json`을 저장 권위로 유지하여 migration 없이 기존 자료와 호환
...
- 로어북 `body_markdown` 원문 편집과 제목·강조·목록·인용·코드·링크·구분선 읽기 렌더링. 원시 HTML 이스케이프와 URL scheme 제한으로 스크립트 실행 차단
