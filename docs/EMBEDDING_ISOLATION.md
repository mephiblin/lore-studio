...
격리 키는 `project_id + namespace + source_role + source_type + source_id + embedding_version + chunk_hash`입니다. `DISCOURSE_REFERENCE`는 별도 index scope이고 factual 검색에서 제외됩니다. 명시 선택 페이지는 검색 점수와 무관하게 포함되며, 설정 후보·참고 문서는 정식 설정 근거로 자동 합류하지 않습니다.
