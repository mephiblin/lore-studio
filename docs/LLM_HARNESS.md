...
`SamplingProfile`은 분량이나 글의 의미를 소유하지 않는 별도 런타임 계층입니다. Writer는 선택한 `temperature/top_p/top_k/frequency_penalty/presence_penalty`를 사용하고, Planner는 strict JSON 안정성을 위해 기존 고정 온도를 유지합니다. Finalizer는 선택값을 그대로 증폭하지 않고 temperature 0.48, top-p 0.86, top-k 32 상한으로 파생합니다. 선택 key·version·override가 반영된 실제 값은 Context pack, 원고 생성 입력, GenerationRun에 함께 저장합니다. 호출별 `max_tokens`는 장문 구간 안전 상한이며 사용자 카드 설정이 아닙니다.
