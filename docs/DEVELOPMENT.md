...
CI는 Ruff/pytest, bundle schema/YAML, Svelte build, Compose config, Mock API Playwright E2E, secret/large-model guard를 실행합니다. 실제 endpoint 시험은 DGX에서 `make test-models`로 별도 수행합니다.
